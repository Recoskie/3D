...
//my linear math functions of rotation

private int coscos(int size,int angle1,int angle2)
{int c1=cos(size,angle1),c2=cos(size,angle2);
int out=c1*c2;try{out=out/size;}
catch(Exception e){out=0;}return(out);}

private int sincos(int size,int angle1,int angle2)
{int s=sin(size,angle1),c=cos(size,angle2);
int out=s*c;try{out=out/size;}
catch(Exception e){out=0;}return(out);}

private int sinsin(int size,int angle1,int angle2)
{int s1=sin(size,angle1),s2=sin(size,angle2);
int out=s1*s2;try{out=out/size;}
catch(Exception e){out=0;}return(out);}

private int sin(int size,int angle){angle=Shift(angle);
angle=ChopSize(size,angle);return(angle);}

private int cos(int size,int angle){angle=Shift(angle+90);
angle=ChopSize(size,angle);return(angle);}
