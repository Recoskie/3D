...
//in
if(e==38){P.Move((P.GetPos()[0]),(P.GetPos()[1]),(P.GetPos()[2]-20));}
//out
if(e==40){P.Move((P.GetPos()[0]),(P.GetPos()[1]),(P.GetPos()[2]+20));}
//left
if(e==37){P.Move((P.GetPos()[0]-20),(P.GetPos()[1]),(P.GetPos()[2]));}
//right
if(e==39){P.Move((P.GetPos()[0]+20),(P.GetPos()[1]),(P.GetPos()[2]));}
repaint();}
...
order=SetOrder(flat,v,azimuth,elevation);
...
//calculate 3D shape
//rotate pionts
float x1=cosT*(x0+px+Data[obj][1])+sinT*(z0+pz+Data[obj][3]);
float y1=-sinTsinP*(x0+px+Data[obj][1])+cosP*(y0+py+Data[obj][2])+cosTsinP*(z0+pz+Data[obj][3]);
float z1=cosTcosP*(z0+pz+Data[obj][3])-sinTcosP*(x0+px+Data[obj][1])-sinP*(y0+py+Data[obj][2]);
...
points[j]=new Point((int)(x1+(width/2)+0.5),(int)(y1+(height/2)+0.5));}
...
g.setColor(new Color(colors[f][0],colors[f][1],colors[f][2],127));
g.fillPolygon(p);}
